(ns poultry.actor
  "PoultryProductionActor — the ISCO-08 6122 community poultry actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one poultry operation request
  ...
  The unconditional invariant: the PoultryProductionAdvisor can never
  directly commit a record the PoultryProductionGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [poultry.advisor :as advisor]
            [poultry.governor :as governor]
            [poultry.store :as store]))

(defn build-graph
  "Build a compiled PoultryProductionActor graph. `store` implements
  `poultry.store/Store`. `advisor` implements
  `poultry.advisor/Advisor` (defaults to `mock-advisor`).
